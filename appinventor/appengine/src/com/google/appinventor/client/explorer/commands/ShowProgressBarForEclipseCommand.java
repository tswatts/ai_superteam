 ...
 */
public class ShowProgressBarForEclipseCommand extends ChainableCommand {
   ...
   */
  public ShowProgressBarForEclipseCommand(String target,ChainableCommand nextCommand, String serviceName) {
   ...
   public ShowProgressBarForEclipseCommand(String target,int buildOption, ChainableCommand nextCommand, String serviceName) {
	   this(target,nextCommand, serviceName);
  ...
  //the main function to be called
    ...
    ode.getProjectService().getEclipseProjectBuildResult(node.getProjectId(), target, this.buildOption, callback);
  }
  ...
  class ProgressBarDialogBox extends DialogBox{
    ...
    ProgressBarDialogBox() {
      ...
      setText("Progress Bar for downloading files "+projectNode.getName());
    ...
    public void addMessages(String projectName, RpcResult result) {
      if (result.succeeded()) {
        show();
        mpb.setProgress(100);
        if (serviceName == "DownloadJavaFilesAction"){
            warningLabel.setHTML("<br />The Java file/s will be saved in the donwnload folder.");
        } else if (serviceName == "DownloadEclipseProjectAction"){
            warningLabel.setHTML("<br />The Eclipse Project will be saved in the donwnload folder.");
        } else {
          warningLabel.setHTML("<br />Waiting for download.");
        }
      } else {
        try {
          ...
          if (currentProgress <= 10) {
            warningLabel.setHTML("<br />Preparing application for download");
            ...
            warningLabel.setHTML("<br />Generating Yail Files");
          } else if (currentProgress <= 55) {
              warningLabel.setHTML("<br />Generating Eclipse project");
          } else if (currentProgress <= 65) {
              warningLabel.setHTML("<br />Generating Java files");
          } else if (currentProgress <= 85) {
              warningLabel.setHTML("<br />Preparing final ouput zip files");
          } else {
            if(serviceName == "DownloadJavaFilesAction"){
                warningLabel.setHTML("<br />The Java file/s will be saved in the donwnload folder.");
            } else if (serviceName == "DownloadEclipseProjectAction"){
