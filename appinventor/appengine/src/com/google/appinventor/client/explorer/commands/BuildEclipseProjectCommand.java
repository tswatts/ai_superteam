 ...
 *
 */
public class BuildEclipseProjectCommand extends ChainableCommand {
  // The build target
  private String target;
  private int buildOption;

  public static final int ECLIPSE_PROJECT_ONLY = 1;
  public static final int JAVA_SOURCES_ONLY = 2;
   ...
   */
  public BuildEclipseProjectCommand(String target) {
    this(target, null);
    this.buildOption = ECLIPSE_PROJECT_ONLY;
  }
   ...
   */
  public BuildEclipseProjectCommand(String target, ChainableCommand nextCommand) {
    super(nextCommand);
    this.target = target;
    this.buildOption = ECLIPSE_PROJECT_ONLY;
  }
  
	public BuildEclipseProjectCommand(String target,int buildOption,
			ChainableCommand nextCommand) {
		super(nextCommand);
		this.target = target;
  ...
  public void execute(final ProjectNode node) {
    ...
    ode.getProjectService().buildEclipseProject(node.getProjectId(), target, nonce, buildOption, callback);
  }
}
